...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    Continent = categorical data

    Life expectancy = numerical data

    ```{r}
    ggplot(gapminder_2007,aes(x=continent,
                              y=lifeExp,
                              fill=continent)) +
      geom_boxplot(outlier= NULL)
    ```
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(gapminder_summary_2007,aes(continent,count))+
      geom_col()
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    *Though both geom_bar() and geom_col() are used to create bar charts, the choice between is dependent on the nature of your data . For a pre-summarized data, if one wants to show counts, we use geom_bar() . If you have raw data with values for each category, use geom_col()*
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    they both are use to visualize categorical and numerical data
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    *it divides the x-axis into a number of breaks, which also show the width of each bar in the histogram plot*
